...
function Library({ tab, setTab, patterns: shown, selected, setSelected, vocabTool, setVocabTool, query, commonIds, commonOnly, setCommonOnly, toggleCommon, masteredIds, toggleMastered }: { tab: LibraryTab; setTab: (t: LibraryTab) => void; patterns: typeof patterns; selected: number; setSelected: (n: number) => void; vocabTool: string; setVocabTool: (v: string) => void; query: string; commonIds:string[]; commonOnly:boolean; setCommonOnly:(v:boolean)=>void; toggleCommon:(id:string)=>void; masteredIds:string[]; toggleMastered:(id:string)=>void }) {
  ...
  return <div className="page"><div className="page-title"><div><div className="eyebrow">REUSABLE LANGUAGE</div><h1>Learning library</h1><p>Language objects connected to real Product Design situations.</p></div><div className="library-overview"><div className="library-total"><b>{vocabulary.length + patterns.length + grammar.length}</b><span>active objects</span></div><button className={commonOnly ? "common-filter active" : "common-filter"} onClick={() => setCommonOnly(!commonOnly)}>★ 常用 <em>{commonIds.length}</em></button></div></div>
    ...
    {tab === "patterns" && (shown.length > 0 ? <div className="library-layout"><div className="object-list">{shown.map((p, i) => <button className={selected === i ? "selected" : ""} key={p.pattern} onClick={() => setSelected(i)}><span className={`object-dot ${p.tone}`} /><div><small>{p.intent}</small><strong>{p.pattern}</strong><em>{p.context}{commonIds.includes(p.id) ? " · ★ 常用" : ""}{masteredIds.includes(p.id) ? " · ✓ 已熟練" : ""}</em></div><b>→</b></button>)}</div><PatternDetail pattern={shown[Math.min(selected, shown.length - 1)]} isCommon={commonIds.includes(shown[Math.min(selected, shown.length - 1)].id)} toggleCommon={toggleCommon} isMastered={masteredIds.includes(shown[Math.min(selected, shown.length - 1)].id)} toggleMastered={toggleMastered} /></div> : <LibraryEmpty commonOnly={commonOnly} />)}
...
function PatternDetail({ pattern, isCommon, toggleCommon, isMastered, toggleMastered }: { pattern: typeof patterns[number]; isCommon:boolean; toggleCommon:(id:string)=>void; isMastered:boolean; toggleMastered:(id:string)=>void }) {
  return <article className="pattern-detail"><div className="detail-top"><span className={`object-dot ${pattern.tone}`} /><div><div className="detail-intent-row"><small>{pattern.intent}</small><button className={isCommon ? "common-toggle active" : "common-toggle"} onClick={() => toggleCommon(pattern.id)}>★ 常用</button></div><h2>{pattern.pattern}</h2><button className="inline-listen" onClick={() => speakEnglish(pattern.pattern)}>◖)) Listen to pattern</button></div></div><div className="meaning">{pattern.meaning}</div><div className="detail-section"><div className="detail-label-row"><label>WORK EXAMPLE</label><button className="inline-listen" onClick={() => speakEnglish(pattern.example)}>◖)) Listen</button></div><p>{pattern.example}</p></div><div className="slot-row"><span>___</span><p>Replace the slots with the evidence and outcome in your own case.</p></div><div className="detail-section"><label>RELATED LEARNING</label><div className="related-chips"><span>reliable baseline</span><span>cause & effect</span><span>Evidence communication</span></div></div><div className="mastery-row"><span>MASTERY</span><b>{isMastered ? "已熟練" : pattern.mastery}</b></div><button className={isMastered ? "mastered-button active" : "mastered-button"} onClick={() => toggleMastered(pattern.id)}>{isMastered ? "✓ 已熟練，可獨立使用" : "標記為已熟練"}</button><button className="primary wide">Practice this pattern →</button></article>;
}
